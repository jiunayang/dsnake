...
#### 爬虫数据导入
项目提供爬虫脚本，可以从网络获取蛇类数据并导入数据库：

```bash
cd backend

# 确保数据库已启动并配置正确的环境变量
export DATABASE_URL="postgresql://snakeuser:snakepass123@localhost:5432/snakedb"
...
脚本会：
1. 检查数据库是否已有数据，询问是否清空
2. 下载预定义的蛇类数据（共20种，包括10种毒蛇和10种无毒蛇）
3. 自动下载图片并转为Base64格式存储
4. 完成后显示导入结果
